...
def plot_latent_2d(m, n):
    ...
    for ax, activation in zip(axes, activations):
        ...
        sns.kdeplot(data=simu_df, x="post0", y="post1", color=palette[2], fill=True, alpha=.35, ax=ax)
        sns.kdeplot(data=recon_df, x="post0", y="post1", color=palette[1], fill=True, alpha=.75, ax=ax)
